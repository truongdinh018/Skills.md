...
- Alternative hosts (Netlify/Vercel/Cloudflare Pages) build on their own infra, so they work even when GitHub Actions is billing-locked. For those, build the static export **without** `PAGES_BASE_PATH` (served at domain root): `BUILD_STATIC_EXPORT=true npm run build` → `out/`. `netlify.toml` configures the Netlify connected-repo build; Vercel can build Next.js natively (no export needed). See README "Deploy cách khác".
