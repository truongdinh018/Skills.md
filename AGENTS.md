...
- Deploy: **GitHub Pages "Deploy from a branch"** (no GitHub Actions). The built static site lives on the orphan branch `cursor/gh-pages-65cf` at root (with `.nojekyll`). Enable once: Settings → Pages → Source "Deploy from a branch" → `cursor/gh-pages-65cf` / root. Refresh the site with `bash scripts/deploy-gh-pages.sh` (rebuilds with `BUILD_STATIC_EXPORT=true PAGES_BASE_PATH=/Skills.md` and force-pushes `out/` to that branch). Static export is **opt-in** and only on when `BUILD_STATIC_EXPORT=true` (see `next.config.ts`); `PAGES_BASE_PATH` sets `basePath` for the project site. Normal `next dev`/`next build`/`next start` are unaffected when those env vars are unset. Caveat: with export enabled, `next start` is unsupported — serve `out/` with any static server. (The old Actions workflow was removed; note GitHub's internal Pages build still runs on Actions infra, so an account billing lock can still block it — in that case use Netlify/Vercel per README.)
